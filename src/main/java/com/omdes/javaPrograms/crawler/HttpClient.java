...
public final class HttpClient {
    ...
    private HttpURLConnection createConnection(String method) {
        ...
        try {
            ...
            if (Config.REQUEST_METHOD_POST.equals(method)) {
                //POST请求方式需要配置
            ...
            httpURLConnection.setRequestMethod(method == null ? Config.REQUEST_METHOD_POST : Config.REQUEST_METHOD_GET);
        } catch (IOException e) {
    ...
    private String response(final HttpURLConnection connection, String encoding) {
        ...
        try {
            if (Config.HTTP_RESPONSE_CODE_SUCCESS == connection.getResponseCode()) {
    ...
    public int sendData(String method, String data) throws IOException {
        ...
        httpURLConnection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded; charset=" + Config.ENCODING);
        httpURLConnection.setRequestProperty("Accept", "text/html");

        this.requestServer(httpURLConnection, method, data, Config.ENCODING);
        this.result = this.response(httpURLConnection, Config.ENCODING);
